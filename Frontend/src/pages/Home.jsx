...
const Home = () => {
  ...
  const profileManager = useRef(null);

  const [isPanelOpen, setIsPanelOpen] = useState(false);
  const [vehiclePanel, setVehiclePanel] = useState(false);
  const [confirmRidePanel, setConfirmRidePanel] = useState(false);
  const [waitingForDriver, setwaitingForDriver] = useState(false);
  const [userProfilShow, setUserProfileShow] = useState(false);
  ...
  useGSAP(() => {
    if (userProfilShow) {
      gsap.to(profileManager.current, { translateX: "0%" });
    } else {
      gsap.to(profileManager.current, { translateX: "100%" });
    }
  }, [userProfilShow]);
  ...
  return (
    <div className="h-screen relative overflow-hidden">
      ...
      <div className="h-screen absolute flex flex-col justify-end top-0 w-full  ">
        <div className="h-[30%] bg-white relative p-5">
          ...
          <h5
            ...
            className="absolute right-12 top-6 text-2xl"
          >
      ...
      <div
        ref={vehiclePanelRef}
        className="w-full fixed z-10 bottom-0 px-3 py-10 pt-12 translate-y-full bg-white"
      >
        <VehiclePanel
      ...
      <div
        ...
        className="w-full fixed z-10 bottom-0 px-3 py-6 pt-12 translate-y-full bg-white"
      >
        <ConfirmRide
      ...
      <div
        ref={profileManager}
        className="w-1/2 h-full fixed z-10 top-0 right-0 px-3 py-6 pt-12 bg-white"
      >
        <UserProfile ride={ride} setUserProfileShow={setUserProfileShow} />
      </div>
    </div>
  );
};
